...
- Затем 3-4 живых предложения — раскрой суть, добавь контекст, сделай интересно
- Заканчивай хэштегами (3-5 штук)
- Добавь источник и ссылку
...
def _fallback_format(item: NewsItem, emoji: str) -> str:
    ...
    hashtags = "#новости #NEURNEWS"
    if item.category == "россия":
        hashtags = "#Россия #РФ #новости #NEURNEWS"
    elif item.category == "мир":
        hashtags = "#МировыеНовости #Мир #новости #NEURNEWS"
    ...
    return (
        ...
        f"{summary}\n\n"
        f"{hashtags}\n\n"
        f"🔗 <a href=\"{item.url}\">Источник: {item.source}</a>"
    )
